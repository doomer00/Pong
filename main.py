...
def rnd_inv(x1, x2, inv1, inv2):
    #This function generates number with certain interva
    return rnd.choice([rnd.uniform(x1, inv1), rnd.uniform(x2, inv2)])

def game_bot(paddle, ball, diff):
    
    speed = 0

    if paddle.y + pad_l / 2 < ball.y and paddle.y + pad_speed <= h - pad_l:
        speed = pad_speed * diff
    elif paddle.y + pad_l / 2 > ball.y and paddle.y - pad_speed >= 0:
        speed = -pad_speed * diff
...
def main():
    ...
    pad1_pos = pyg.Vector2(10, h/2 - pad_l/2)
    pad2_pos = pyg.Vector2(w - 10 - pad_w, h/2 -  pad_l/2)
    pad_speed = 0.5
    

    dx, dy = rnd_inv(-1, 1, -0.4, 0.4), rnd.uniform(1,-1)
    previous = ball_pos
